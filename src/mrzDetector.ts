...
export type MRZFeatures = {
  lineCountScore: number;
  lineLengthScore: number;
  horizontalDensityScore: number;
  characterCompatibilityScore: number;
  lineSpacingScore: number;
  alignmentScore: number;
  pLessThanScore: number;
  nameSeparatorScore: number;
  lowerPositionScore: number;
};

export type MRZCandidate = {
  format: "TD3";
  boundingBox: BoundingBox;
  confidence: number;
  features: MRZFeatures;
  source?: "p-anchor" | "visual" | "ocr";
  text?: string;
  rotation?: number;
};
...
export function normalizeMRZText(
  text: string
): string {
  return text
    .toUpperCase()
 ...
 */
export function findPAnchors(
  lines: OCRLine[],
  imageWidth: number,
  imageHeight: number
): MRZCandidate[] {
  const candidates: MRZCandidate[] =
    [];
...
export function scoreOCRPair(
  ...
  const characterCompatibilityScore =
    (
      mrzCharScore(
        firstText
      ) +
 ...
 */
export function detectMRZFromOCR(
  lines: OCRLine[],
  imageWidth: number,
  imageHeight: number
): MRZCandidate[] {
...
export function detectMRZCandidates(
  image: ImageData
): MRZCandidate[] {
  const bands =
    getBands(image);
