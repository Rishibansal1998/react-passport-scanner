...
import {scoreVisualPair,paddedCrop} from '../.test-build/mrzDetector.js';
...
const score=(p,h=1000)=>scoreVisualPair(p[0],p[1],1000,h);
...
test('close line spacing is preferred',()=>assert.ok(score(pair()).confidence>(score([{...pair()[0]},{...pair()[1],y:180}])?.confidence??0)));
